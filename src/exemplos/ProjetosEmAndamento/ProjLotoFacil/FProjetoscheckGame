package ProjetosEmAndamento.ProjLotoFacil;
    ...
    public static void main(String[] args) {
        ...
        String line = sc.nextLine();// manter leitura de varios dados na mesma linha
            for (int i = 0; i < num_dia.length; i++) {
                num_dia[i] = Integer.parseInt(line.substring(i * 2, i * 2 + 2));// pega a linha acima em String e coloca em varias matrizes/indice, depois faz sub classes
        }

        //varias matrizes/indice,sub classes| String[] numbers = line.split(" ");

        /** loop para verificar número a número da variavel num_dia | loop para verificar número a número da variavel meus_num1
        depois pela a variavel e seus indices e converte de String para int if para caso meus_num e seus indices e num_dia[] 
        forem parecidos guardarem em numero_repetido e incrementar ++; */

            for (int i = 0; i < num_dia.length; i++) { // converte string para int | num_dia[i] = Integer.parseInt(numbers[i]); */
